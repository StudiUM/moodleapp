...
if [ -n "$2" ]; then
  echo "Erreur: STUDIUM_ENV n'est plus supporte. Utilisez uniquement DEVICE_TARGET ('android' ou 'ios') en 1er argument."
  exit 1
fi
...
# Déterminer le fichier source uniquement selon la plateforme.
...
if [ -z "${STUDIUM_NAME:-}" ] || [ -z "${STUDIUM_URL:-}" ]; then
  echo "Erreur: STUDIUM_NAME et STUDIUM_URL sont obligatoires et doivent avoir le meme nombre d'elements, separes par '|', par exemple STUDIUM_NAME='Site A|Site B' et STUDIUM_URL='https://a|https://b'."
  exit 1
fi
